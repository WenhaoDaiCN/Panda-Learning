...
def show_score(cookies):
    ...
    print("阅读文章:{}/6,观看视频:{}/6,登陆:{}/1,文章时长:{}/8,视频时长:{}/10".format(*each))
    return total, each
...
def article(cookies, a_log, each):
    if each[0] < 6 or each[3] < 8:
        ...
        while True:
            if each[0] < 6 and try_count < 10:
                ...
                for i in range(a_log, a_log + a_num):
                    ...
                    for j in range(240):
                        driver_article.go_js('window.scrollTo(0, document.body.scrollHeight/240*{})'.format(j))
                        print("\r文章学习中，文章剩余{}篇,本篇剩余时间{}".format(a_log + a_num - i, 240 - j), end="")
                        time.sleep(1)
        ...
        try_count = 0
        while True:
            if each[3] < 8 and try_count < 10:
                num_time = 60
                if is_active.is_active():
                    num_time = 30
                driver_article.get_url(links[a_log])
                for i in range((8 - each[3]) * 4 * num_time):
                    driver_article.go_js(
                        'window.scrollTo(0, document.body.scrollHeight/{}*{})'.format((8 - each[3]) * 4 * num_time, i))
                    print("\r文章学习中，文章总时长剩余{}秒".format((8 - each[3]) * 4 * num_time - i), end="")
                    time.sleep(1)
                    if i % (240) == 0 and i != (8 - each[3]) * 4 * num_time:
                        total, each = show_score(cookies)
                        if each[3] >= 8:
    ...
    if each[1] < 6 or each[4] < 10:
        ...
        while True:
            if each[1] < 6 and try_count < 10:
                v_num = 6 - each[1]
                if is_active.is_active():
                    v_num = math.ceil(v_num / 2)
                for i in range(v_log, v_log + v_num):
                    driver_video.get_url(links[i])
                    for j in range(5 * 60):
                        driver_video.go_js('window.scrollTo(0, document.body.scrollHeight/300*{})'.format(j))
                        print("\r视频学习中，视频剩余{}个,本次剩余时间{}".format(v_log + v_num - i, 300 - j), end="")
                        time.sleep(1)
        ...
        try_count = 0
        while True:
            if each[4] < 10 and try_count < 10:
                num_time = 60
                if is_active.is_active():
                    num_time = 30
                driver_video.get_url(links[v_log])
                for i in range((10 - each[4]) * 5 * num_time):
                    driver_video.go_js(
                        'window.scrollTo(0, document.body.scrollHeight/{}*{})'.format((10 - each[4]) * 5 * num_time, i))
                    print("\r视频学习中，视频总时长剩余{}秒".format((10 - each[4]) * 5 * num_time - i), end="")
                    time.sleep(1)
                    if i % (300) == 0 and i != (10 - each[4]) * 5 * num_time:
                        total, each = show_score(cookies)
                        if each[4] >= 10:
                            print("检测到视频时长分数已满,推出学习")
                            break
                driver_video.go_js('window.scrollTo(0, document.body.scrollHeight)')
                total, each = show_score(cookies)
            else:
